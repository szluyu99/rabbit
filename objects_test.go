...
func TestOnCreate(t *testing.T) {
	...
	err = c.CallPut("/user", tuser{Name: "not_dangerous"}, nil)
	assert.Nil(t, err)
}
...
func TestOnUpdate(t *testing.T) {
	...
	err := c.CallPatch("/user/1", map[string]any{"name": "not_dangerous"}, nil)
	...
	err = c.CallPatch("/user/2", map[string]any{"name": "not_dangerous"}, nil)
	assert.Nil(t, err)
}

func TestOnBatchDelete(t *testing.T) {
	c, _ := initHookTest(t)

	err := c.CallDelete("/user", []string{"1", "2", "3"}, nil)
	assert.NotNil(t, err)

	err = c.CallDelete("/user", []string{"2", "3"}, nil)
	assert.Nil(t, err)
}
		...
		Views: []QueryView{
			{
				...
				Prepare: func(db *gorm.DB, ctx *gin.Context) (*gorm.DB, *QueryForm, error) {
					return db, &QueryForm{Page: 1, Limit: -1, ViewFields: []string{"ID", "Name"}}, nil
				},
			},
		},
	}
